...
public class Skeletonize3D_ implements PlugInFilter
{
    ...
    public void prepareData(ImageStack outputImage)
    {
        ...
        for (int x = 0; x < width; x++)
            for (int y = 0; y < height; y++)
                if (((byte[]) this.inputImage.getPixels(1))[x + y * width] != 0)
                    ((byte[]) outputImage.getPixels(1))[x + y * width] = 1;
    ...
    {
        //return Array.getShort(this.inputImage.getPixels(z + 1), x + y * this.width);
