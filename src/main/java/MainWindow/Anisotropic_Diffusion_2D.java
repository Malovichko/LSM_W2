...
public class Anisotropic_Diffusion_2D extends Thread
{
    ...
    public Anisotropic_Diffusion_2D(Label processLabel){
        this.processLabel = processLabel;
    }
    public Anisotropic_Diffusion_2D(){
    ...
    public boolean show_window(boolean param)
    {
        stack = imp.getStack();
...
//				G = new float[width][height][3]; // must be clean for each slice
                ...
                // create new image if you break the cycle with Esc, if it is saving step, or if we get below stopping condition
                {
                    ...
                    if (scount == 1) {
                        ...
                        if (imp2==null) {
                            ...
                            imp2 = new ImagePlus(imp.getShortTitle()+" dif"+(iter+1)+((breaked)?"-breaked":""), stack2);
                            imp2.show();
                        } else {
                            stack2.addSlice("iter"+(iter+1), ip2);
                            imp2.setStack(null, stack2);
                            imp2.setSlice(imp2.getStackSize());
                            imp2.setTitle(imp.getShortTitle()+" dif"+(iter+1));
                        }
                    } else
        ...
        IJ.showProgress(1.0);
        if (scount>1) new ImagePlus(imp.getShortTitle()+" dif"+nb_iter, stack2).show();
        if (processLabel != null)
            processLabel.setText("Diffusion Finished");
    ...
    public void printProgress(int sliceNumber){

        if (processLabel != null)
        processLabel.setText("Diffusion processing: Slice " + sliceNumber);
    }

    ImageStack getResultStack(){
        return stack2.duplicate();
    }
}