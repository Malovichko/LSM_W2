...
public class WindowLevelAdjuster implements PlugIn, Runnable {
    protected ImagePlus imp;
    protected ImageProcessor[] ips; // all for stack
    protected int nSlices;
    protected GenericRecallableDialog gd;
    protected String Label;
    protected float cur_window, cur_level, defaultWindow = 0, defaultLevel = 0;
    ...
    public void run(String arg) {
        ...
        if (imp==null) {
            IJ.showMessage("Error: no image defined..");
            return;
        }
        Label = "Window/Level Adjuster";
        ...
        gd = new GenericRecallableDialog(
                "Window/Level Adjuster", IJ.getInstance());
    ...
    void initWindowAndLevel() {
        ...
        for (int i=1; i<=nSlices; i++) {
            ...
            imgMax = Math.max(imgMax, (float)ips[i-1].getMax());//getRegionMax(ips[i-1], rect2));
            imgMin = Math.min(imgMin, (float)ips[i-1].getMin());//getRegionMin(ips[i-1], rect2));
        ...
        cur_window = defaultWindow = imgMax - imgMin;
        cur_level = defaultLevel = imgMin + cur_window/2f;
        //System.out.println(cur_window + " " + cur_level);
        for (int i=1; i<=nSlices; i++)
            setWindowAndLevel(ips[i-1]);
    ...
    void setWindowAndLevel(ImageProcessor ip) {
        double min = cur_level - cur_window / 2;
        double max = cur_level + cur_window / 2;
